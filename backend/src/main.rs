...
async fn main() -> Result<()> {
    ...
    // 若配置里 VoLTE 主开关已开启，则随服务启动 supervisor worker。
    // 关闭状态下什么都不做，不会碰调制解调器。
    {
        let volte = Arc::clone(&app_state.volte_manager);
        tokio::spawn(async move {
            volte.spawn_worker().await;
        });
    }
    ...
    let device_routes = Router::new()
        ...
        .route(
            "/api/volte/sms",
            post(set_volte_sms_handler).options(options_handler),
        )
        .route(
            "/api/volte/settings",
            post(set_volte_settings_handler).options(options_handler),
        )
        .route(
            "/api/volte/refresh",
            post(refresh_volte_handler).options(options_handler),
        )
        .route(
