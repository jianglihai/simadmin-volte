...
pub struct VolteConfig {
    /// 主开关。关闭时 supervisor 不启动 worker。
    #[serde(default)]
    pub feature_enabled: bool,
    /// 注册成功后是否启用 IMS 短信收发。
    #[serde(default)]
    pub sms_enabled: bool,
    /// IMS PDP 上下文协议：IPV4V6 / IP / IPV6。
    #[serde(default = "default_volte_apn_protocol")]
    pub apn_protocol: String,
    /// 漫游时是否允许注册。
    #[serde(default)]
    pub roaming_allowed: bool,
    /// 数据槽位意图：independent_wwan1 / secondary_qmi_data。
    #[serde(default = "default_volte_data_path")]
    pub data_path_intent: String,
}

fn default_volte_apn_protocol() -> String {
    "IPV4V6".to_string()
}

fn default_volte_data_path() -> String {
    "independent_wwan1".to_string()
}
...
impl Default for VolteConfig {
    fn default() -> Self {
        Self {
            ...
            sms_enabled: false,
            apn_protocol: default_volte_apn_protocol(),
            roaming_allowed: false,
            data_path_intent: default_volte_data_path(),
        }
    }
}
...
impl ConfigManager {
    ...
    pub fn set_volte_config(&self, volte: VolteConfig) -> Result<(), String> {
        {
            let mut c = self.config.write().unwrap();
            c.volte = volte;
        }
