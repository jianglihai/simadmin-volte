...
/** 原生 VoLTE 运行时状态。字段与后端 VolteRuntimeStatusResponse 一一对应。 */
export interface VolteRuntimeStatus {
  /** disabled | starting | registered | degraded | stopping */
  phase: string
  /** starting | identity | identity_aka | radio | modem | bearer | pcscf | register_ipsec | register_udp | registered | stopping */
  stage: string
  ...
  data_path_mode?: string | null
  imsi?: string | null
  home_domain?: string | null
  public_identity?: string | null
  pcscf?: string | null
  ue_address?: string | null
  own_number?: string | null
}

/** GET /api/volte/control 的响应：配置 + 运行时。 */
export interface VolteControl {
  feature_enabled: boolean
  sms_enabled: boolean
  /** IPV4V6 | IP | IPV6 */
  apn_protocol: string
  roaming_allowed: boolean
  /** independent_wwan1 | secondary_qmi_data */
  data_path_intent: string
  runtime: VolteRuntimeStatus
}

/** POST /api/volte/settings 请求体，字段全部可选（部分更新）。 */
export interface VolteSettingsRequest {
  apn_protocol?: string
  roaming_allowed?: boolean
  data_path_intent?: string
}
